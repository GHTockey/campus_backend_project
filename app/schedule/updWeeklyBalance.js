...
module.exports = {
    ...
    async task(ctx) {
        ...
        try {
            ...
            uList.forEach(el => {
                // 今日余额减去昨日余额 == 今天赚了多少钱
                let newItem = +(el.money - el.weekly_balance[el.weekly_balance.length - 1]).toFixed(2) < 0 ? 0 : +(el.money - el.weekly_balance[el.weekly_balance.length - 1]).toFixed(2)
                el.weekly_balance.push(newItem)
                el.weekly_balance.shift()
                sql += ` WHEN ${el.user_id} THEN '[${el.weekly_balance}]'`
            });
            ctx.app.mysql.query(`UPDATE user_wallet SET weekly_balance = CASE user_id ${sql} END WHERE user_id IN (${ids});`)
        } catch (error) {
